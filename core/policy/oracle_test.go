...
func Test_Oracle_AddPolicies_WhenEndpointFails(t *testing.T) {
	repo := NewRepository()
...
func createFilledOracle(mockServerURL string, interval time.Duration, repo *Repository) *Oracle {
	...
	oracle.fetchSubscriptions = append(
		oracle.fetchSubscriptions,
		policySubscription{
			policy:      oracle.Policy("1"),
			subscribers: []*Repository{repo},
		},
		policySubscription{
			policy:      oracle.Policy("2"),
			subscribers: []*Repository{repo},
		},
	)
	return oracle
