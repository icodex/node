...
func Test_iptablesDNSFirewall_Setup(t *testing.T) {
	...
	assert.True(t, mockedIpset.VerifyCalledWithArgs("create myst-provider-destinations-whitelist hash:ip"))
	assert.True(t, mockedIptables.VerifyCalledWithArgs("-N PROVIDER_DNS_FIREWALL"))
	assert.True(t, mockedIptables.VerifyCalledWithArgs("-A PROVIDER_DNS_FIREWALL -m set --match-set myst-provider-destinations-whitelist dst -j ACCEPT"))
...
func Test_iptablesDNSFirewall_Teardown(t *testing.T) {
	...
	firewall := &incomingBlockerIptables{}
	firewall.Teardown()
	assert.True(t, mockedIpset.VerifyCalledWithArgs("destroy myst-provider-destinations-whitelist"))
		...
		mocks: map[string]iptablesExecResult{
			...
			"-S PROVIDER_DNS_FIREWALL": {
				output: []string{
					...
					"-A PROVIDER_DNS_FIREWALL -m set --match-set myst-provider-destinations-whitelist dst -j ACCEPT",
...
func Test_iptablesDNSFirewall_AllowIPAccess(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.True(t, mockedIpset.VerifyCalledWithArgs("add myst-provider-destinations-whitelist 1.2.3.4"))

	err = removeRule()
	assert.NoError(t, err)
	assert.True(t, mockedIpset.VerifyCalledWithArgs("del myst-provider-destinations-whitelist 1.2.3.4"))
}
